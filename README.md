# [パワーポイントの「左揃え」のような「整列」機能がある付箋アプリ](https://github.com/Yz-Filer/fusen)  
...
- Ctrl + 付箋を左クリック：付箋を選択状態／選択状態解除にする  
...
- 固定間隔値を設定：「左右に整列（固定間隔）」「上下に整列（固定間隔）」「左右均等割り」の時の付箋間の間隔をピクセル単位で指定する  
  ※「0」に設定すると付箋同士が隣接します。  
...
> [!WARNING]  
> 「左右均等割り」は付箋（画像）には対応してません。  
> 「整列時画像の余白無視」は変形時には適用されません。  

### 5.5 タスクトレイアイコンの右クリックメニュー  
